 ...
 */
var QEDITOR_ALLOW_TAGS_ON_PASTE, QEDITOR_DISABLE_ATTRIBUTES_ON_PASTE, QEDITOR_TOOLBAR_HTML, QEDITOR_TOOLTIP_HTML;

QEDITOR_TOOLTIP_HTML = "<div class=\"qeditor_tooltip\" contenteditable=\"false\">\n    <input type=\"text\" autocomplete=\"off\">\n    <div class=\"actions\">\n        <a href=\"javascript:;\" class=\"cancel\">Cancel</a>\n        <a href=\"javascript:;\" class=\"insert\">Insert</a>\n    </div>\n</div>";
...
window.QEditor = {
  ...
  action: function(el, a, p) {
    ...
    if ((a === "createLink" || a === "insertImage") && !p) {
      editor.parent().find('.qeditor_tooltip').data({
        el: el,
        a: a
      }).show().find('input').val('http://').focus();
      return false;
    }
    if (QEditor.state(a)) {
  ...
  exitFullScreen: function() {
    $(".qeditor_border").removeClass("qeditor_fullscreen").data("qe-fullscreen", "0").find(".qe-fullscreen span").attr("class", "fa fa-arrows-alt");
    return $(".qeditor_border").find(".qeditor_preview").focus();
  },
...
(function($) {
  return $.fn.qeditor = function(options) {
    return this.each(function() {
      var currentVal, editor, obj, placeholder, qe_heading, toolbar, tooltip;
      obj = $(this);
      ...
      tooltip.find('.insert').click(function() {
        var data, val;
        val = tooltip.find('input').val().replace('javascript:', '');
        if (val.indexOf('http://') === -1 && val.indexOf('https://') === -1 || val.length <= 8) {
          return false;
        }
        data = tooltip.data();
        return QEditor.action(data.el, data.a, val);
      });
      editor.before(tooltip);
      return editor.focus(function() {
        return tooltip.hide();
      });
    });
  };
